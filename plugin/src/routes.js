...
/**
 * 构造桌宠路由族。
 * @param {{service: import('./chat.js').PetChatService, writeBridge: () => string}} deps
 * @returns {import('@deepseek-ai/dsh-host-webserver').WebRoute[]}
 */
export function makePetRoutes({ service, writeBridge }) {
  return [
    {
      kind: 'exact',
      path: '/api/pet/health',
      handler: (req, res) => {
        ...
        json(res, 200, { ok: true, service: 'desktop-pet', port: req.socket?.localPort ?? null })
      },
    },
    {
      kind: 'exact',
